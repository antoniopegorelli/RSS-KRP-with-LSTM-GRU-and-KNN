...
def add_data():
    ...
    ap_noise = pd.Series(np.random.normal(0, 1, 6))
    ...
    data_result = data_result.append({'test' : test, 'time' : (supervisor.getTime() // 1 - test_time // 1), 'posX' : trans_values[2], 'posY' : trans_values[0], 'KRP_period' : KRP_period, 'ap1' : ap_signals.iloc[0], 'ap2' : ap_signals.iloc[1], 'ap3' : ap_signals.iloc[2], 'ap4' : ap_signals.iloc[3], 'ap5' : ap_signals.iloc[4], 'ap6' : ap_signals.iloc[5]}, ignore_index=True)
